...
from InputSchema import SchemaPrep

data_p = SchemaPrep(csv_col=None,test_size=None, random_state=None)
...
# Categorical columns with vocab size (type of values)
...
def model_fn(mode,
    ...
    """
    global data_p
    label_values = tf.constant(data_p.LABELS)
    ...
    with tf.variable_scope('embeddings', initializer=embed_initializer):
        ...
        for col, vals in data_p.CATEGORICAL_COLS:
    ...
    for col in data_p.CONTINUOUS_COLS:
    ...
    # Build the DNN
    curr_layer = inputs
    hiddenunitdepth=len(hidden_units)
    cnt=1
    for layer_size in hidden_units:
        activateionMethod= tf.nn.relu if(cnt == hiddenunitdepth) else tf.nn.sigmoid

        curr_layer = tf.layers.dense(
            curr_layer,
            layer_size,
            activation=activateionMethod,
    ...
    if mode == TRAIN:
        ...
        train_op = tf.train.FtrlOptimizer(learning_rate=learning_rate,l1_regularization_strength=3.0,l2_regularization_strength=10.0).minimize(cross_entropy, global_step=global_step)
...
def csv_serving_input_fn(default_batch_size=None):
    ...
    features.pop(data_p.LABEL_COLUMN)
...
def example_serving_input_fn(default_batch_size=None):
    ...
    global data_p
    feature_spec = {}
    for feat in data_p.CONTINUOUS_COLS:
        feature_spec[feat] = tf.FixedLenFeature(shape=[], dtype=tf.int64)

    for feat, _ in data_p.CATEGORICAL_COLS:
...
def parse_csv(rows_string_tensor):
    ...
    global data_p
    columns = tf.decode_csv( rows_string_tensor, record_defaults=data_p.CSV_COLUMN_DEFAULTS)



    features = dict(zip(data_p.CSV_COLUMNS, columns))
...
def input_fn(filenames,
    ...
    """
    global data_p
    #print(data_p.CSV_COLUMNS)
    #data_p.defineDefaultLabel(filenames[0])
    ...
    return features, features.pop(data_p.LABEL_COLUMN)


def prepareDataSet(inputfile,outputFile):
    global data_p
    data_p.read_DataSet(inputfile)

    data_p.outputDataSet(outputFile)

    trainfileName=outputFile+".train.csv"
    testfileName=outputFile+".test.csv"
